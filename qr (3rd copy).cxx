...
int main(int argc, char** argv) {
	...
	resultorder.assign(num_analysis, -1);
	...
	size_t offset = 0, start_analysis = 0, start_bins = 0;
	...
	#pragma omp parallel
	{
		...
		if(!ch._load)
		{
			#pragma omp single	
			{	
				OutputHandler::setup_outputfiles(ch, minparval, maxparval);
			...
			//~ #pragma omp single
				...
				//~ {
					...
					//~ {
						...
						{
							OutputHandler::write_binresult(num_ipol, ch, rh, fh, center);
...
//~ 
						...
						if(ch._outdotflag)
						{
							OutputHandler::write_dotproduct(num_ipol, rh, fh, center, distances);
...
//~ 
						...
						if(ch._summaryflag)
						{
							...
							{
								OutputHandler::write_summary(fh, rh);
		...
		else
		{
			...
			//~ for(size_t num_ipol = 708; num_ipol < 710; num_ipol++)
			{
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				if(ch._outdotflag)
				{
					OutputHander::write_dotproduct(num_ipol, rh, fh, center, distances);
...
//~ 
			...
			//~ 
				...
				if(ch._summaryflag)
				{
					#pragma omp critical (summaryout)
					{
						OutputHander::write_summary(fh, rh);
				...
				//~ if(!resultval[start_bins + offset].empty() && !resulterr[start_bins + offset].empty() && resultorder[start_bins + offset] != -1)
				#pragma omp critical (ipolresultoutput)
				{
					ofstream output;
					output.open(ch._filename, ofstream::out | ofstream::app);
